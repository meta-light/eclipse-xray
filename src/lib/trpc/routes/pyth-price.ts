...
export const pythPrice = t.procedure
    .query(async () => {
        ...
        try {
            ...
            return price && typeof price.price === 'string' && typeof price.expo === 'number' ? parseFloat(price.price) * Math.pow(10, price.expo) : null;
        } 
        catch (error) {console.error("Error fetching ETH/USD price:", error); return null;}
    });