...
<div>
    <div class="relative mb-5 block w-full rounded-lg border-transparent bg-black px-3">
        {#if data.changes}
            <Namor text={data.account} let:result>
                <div>
                    <div class="flex justify-between">
                        <div>
                            <div class="flex items-center">
                                <a href="/account/{data.account}" class="pointer-events-auto border-x-0 border-t-0 border-dotted hover:link-success">
            ...
            {#each data.changes as change}
                {#if change.amount}
                    <TokenProvider address={change.mint} let:metadata let:tokenIsLoading>
                        {#if tokenIsLoading}
                        ...
                        {:else if metadata?.image}
                            <div class="mb-5 ml-3 w-full rounded-lg text-left" in:fade={{duration: 500}}>
                                <div class="pointer-events-none relative grid grid-cols-12 items-center gap-3 rounded-lg">
                                    <div class="col-span-">
                                        <a href="/token/{metadata.address}" class="pointer-events-auto block transition-transform hover:scale-125">
