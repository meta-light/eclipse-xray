<script lang="ts">
    ...
    $: metadataHTML = formatHighlight(JSON.stringify(data || {}, null, 2), {keyColor: "#a5a3a3", numberColor: "#e8a034", stringColor: "#24ae67",});
    let showCode = true;
</script>
...
<div class="mb-3" in:fade={{duration: 500}}>
    <div class="mt-3 grid grid-cols-12 items-center gap-3 rounded-lg p-1">
        ...
        {#if showCode || label === "token" || label === "update-cNFT"}
            <div class="code col-span-12 w-full overflow-hidden px-3 border border-black border rounded-lg">
