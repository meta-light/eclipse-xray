<script lang="ts">
    ...
    onMount(() => {const savedIsMainnet = localStorage.getItem("isMainnet"); if (savedIsMainnet !== null) {isMainnetValue = JSON.parse(savedIsMainnet);}});
    onMount(() => {const params = new URLSearchParams(window.location.search); const network = params.get("network"); isMainnetValue = network !== "devnet"; localStorage.setItem("isMainnet", JSON.stringify(isMainnetValue));});
