<style>
    .text-sky {color: text-info-content;}
    .text-tangerine {color: text-#e8a034;}
</style>
<script lang="ts">
...
<div class="pt-0">
    {#if hasNonEmptyLogs}
        {#each parsedLogs as instruction, idx}
            {#if idx === 0}
                <p class="px-3">
                    <span class="mb-1">{`#${idx + 1} - `}</span>
                    <span>{`${instruction.invokedProgram} Instruction`}</span>
                </p>
            {:else}
...
{#if totalComputeUnits > 0}
    ...
    <div class="px-3 pt-1 text-sm">{`${totalComputeUnits} compute units consumed`}</div>
{/if}
