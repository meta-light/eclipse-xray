<script lang="ts">
    ...
    if (address) {account = client.niftyAsset.createQuery([address, isMainnetValue ? "mainnet" : "devnet"], {refetchOnMount: false, refetchOnWindowFocus: false,});}
    $: if ($account && $account.data) {status = { isError: $account.isError, isLoading: $account.isLoading }; updateAsset($account.data[0]);}
    const updateAsset = (data: UINiftyAsset) => {
        asset = data;
        if (asset) {
            const metadata = getExtension(asset, ExtensionType.Metadata);
            if (metadata && metadata.uri) {(async () => { try {asset.json = await fetchJsonMetadata(metadata.uri);} catch (error) {console.error("Error in fetchJsonMetadata:", error);}})();}
        }
    };
    ...
    const fetchJsonMetadata = async (jsonUri: string) => {
        try {
            ...
            if (!contentType || !contentType.includes("application/json")) {throw new TypeError("Received non-JSON content type");}
