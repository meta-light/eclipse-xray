<script lang="ts">
    ...
    export const metadata: UITokenMetadata & {
        mintExtensions?: { metadata?: { name?: string; uri?: string } };
    } = {
        address: "",
        attributes: [],
        collectionKey: "",
    ...
    $: if (address === SOL) {
        ...
        metadata.image = data?.content.links?.image || data?.content.files?.find((file: any) => file.mime?.startsWith("image/"))?.uri;
        metadata.name = data?.content.metadata?.name;
        metadata.files = data?.content.files;
        metadata.video_uri = data?.content.files?.find((file: any) => file.mime?.startsWith("video/"))?.uri;
    ...
    $: if (metadata.mintExtensions) {
        ...
        } else if (jsonUri) {metadata.image = jsonUri;}
    }

    $: tokenIsLoading = (address !== SOL && $asset?.isLoading) || (address !== SOL && status?.isLoading);
